...
For a given source schema, there must be exactly one non-deprecated lookup field that can resolve
a given entity by a specific key. If multiple non-deprecated lookup fields are defined that
